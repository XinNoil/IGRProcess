...
def convert_RTKLite_log(trip_dir, rtk_file_name):
    gngga_f = open(f"{trip_dir}/GNGGA.csv", "w")
    gngga_f.write(HEADER_DEF["GNGGA"])
    gngga_f.write("\n")

    with open(f"{trip_dir}/supplementary/{rtk_file_name}", 'r', encoding='utf-8') as f:
        while (line := f.readline()):
            # 空格前后是时间戳和实际记录项
            tempList = line.split("     ")
...
def convert_one_dir(trip_dir):
    ...
    for now_name in os.listdir(os.path.join(trip_dir, "supplementary")):
        if now_name.startswith("gnss_log") and now_name.endswith(".txt"):
            gnss_file_name = now_name
            convert_GNSS_log(trip_dir, gnss_file_name)

        elif now_name.endswith("gngga.txt"):
            rtk_file_name = now_name
