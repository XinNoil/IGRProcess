...
def process_paths(_infos):
    for info in _infos:
        ...
        info_dict = {'area': _area, '_people':_people}
        path_dir = os.path.join('processed', _device, trace, 'supplementary')
        os.makedirs(path_dir, exist_ok=True)

        if args.subtype is not None:
            subtype = info_dict[args.subtype]
            if osp.isfile(os.path.join('origin', _device, subtype, rnx_file)):
                print(f"{os.path.join('origin', _device, subtype, gnss_file)}")
                shutil.copyfile(os.path.join('origin', _device, subtype, gnss_file), os.path.join(path_dir, gnss_file))
            if osp.isfile(os.path.join('origin', _device, subtype, rnx_file)):
                print(f"{os.path.join('origin', _device, subtype, rnx_file)}")
                shutil.copyfile(os.path.join('origin', _device, subtype, rnx_file), os.path.join(path_dir, rnx_file))
            if osp.isfile(os.path.join('origin', _device, subtype, sensor_file)):
                print(f"{os.path.join('origin', _device, subtype, sensor_file)}")
                shutil.copyfile(os.path.join('origin', _device, subtype, sensor_file), os.path.join(path_dir, sensor_file))
